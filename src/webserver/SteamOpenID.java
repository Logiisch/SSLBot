...
public class SteamOpenID {
     ...
     */
    public String verify(String receivingUrl, Map<String,String[]> responseMap) {
        if (this.discovered == null) {
            return null;
        }

        //responseMap = removePort(responseMap);
        //receivingUrl = removePort(receivingUrl);
        receivingUrl = matchhttps(receivingUrl,responseMap);
    ...
    private String matchhttps(String retURL, Map<String,String[]> in) {
        for (String key:in.keySet()) {
            if (!key.equalsIgnoreCase("openid.return_to")) {

                continue;
            }
            String value = in.get(key)[0];
            if (value.startsWith("https://")) {
                return retURL.replace("http://","https://");
            } else {
                return retURL.replace("https://","http://");
            }
